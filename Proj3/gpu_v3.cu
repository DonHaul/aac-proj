...
#define GET_TIME(X, Y) (((Y).tv_sec - (X).tv_sec) + ((Y).tv_nsec - (X).tv_nsec) / 1000000000.0)
  ...
  __constant__ __device__ float is_d;
  ...
  __global__ void ezCalc ( float *ez, float *hx, float *hy ) {
    ...
    if (i == is_d) {
      ez[j * IE_d + i] = cos(2 * pi_d * freq_d * n_d * dt_d);
    } else {
      if (j == 0) { // at x=0
  ...
  int main(int argc, char * argv[]) {
    ...
    cudaMemcpyToSymbol(is_d, &is, sizeof(float), 0, cudaMemcpyHostToDevice);
    ...
    for (n = 0; n < nsteps; n++) { // TIME
      ...
      cudaMemcpyToSymbol(n_d, &n, sizeof(float), 0, cudaMemcpyHostToDevice);
      ...
      printf("\n======Part 2: (%f)====", GET_TIME(Step1, End));
    }

    printf("\n\n====Total time: %f\n", accum);

    // write output to file
    fp = fopen("output_gpu_v3.txt", "w");
